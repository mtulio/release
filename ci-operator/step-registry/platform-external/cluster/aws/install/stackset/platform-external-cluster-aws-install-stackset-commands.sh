...
TEMPLATES+=( "stacks/deploy-upi-public-ha" )
...
STACK_NAMES="${SHARED_DIR}/aws_cfn_stacks"
# mkdir -vp $STACK_NAMES
touch "$STACK_NAMES"
...
# TODO(mtulio): do we need OPENSHIFT_INSTALL_INVOKER?
...
IGNITION_CA="$(jq '.ignition.security.tls.certificateAuthorities[0].source' ${SHARED_DIR}/master.ign)"
...
==================
CREATING StackSet:
deploy-upi-public-ha
=================="

cf_params_stackset=${ARTIFACT_DIR}/cf_params_upi.json
add_param_to_json AvailabilityZoneCount 2 "${cf_params_stackset}"
add_param_to_json NamePrefix "${INFRA_ID}" "${cf_params_stackset}"
add_param_to_json TemplatesBaseURL "TODO" "${cf_params_stackset}"
...
echo "${VPC_STACK_NAME}" >> "${STACK_NAMES}"
...
echo "${INFRA_STACK_NAME}" >> "${STACK_NAMES}"
...
echo "${SECURITY_STACK_NAME}" >> "${STACK_NAMES}"
aws cloudformation create-stack \
...
echo "${BOOTSTRAP_STACK_NAME}" >> "${STACK_NAMES}"
aws cloudformation create-stack \
...
echo "${CONTROL_PLANE_STACK_NAME}" >> "${STACK_NAMES}"
aws cloudformation create-stack \
...
do
  ...
  echo "${COMPUTE_STACK_NAME}" >> "${STACK_NAMES}"
  aws cloudformation create-stack \
